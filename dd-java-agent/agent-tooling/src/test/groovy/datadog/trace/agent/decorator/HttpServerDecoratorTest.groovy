...
class HttpServerDecoratorTest extends ServerDecoratorTest {
  ...
  def "test onRequest"() {
    ...
    then:
    if (req) {
      1 * span.setAttribute(AttributeNames.HTTP_METHOD, "test-method")
      1 * span.setAttribute(AttributeNames.HTTP_URL, url)
    }
    0 * _
  ...
  def "test url handling for #url"() {
    ...
    1 * span.setAttribute(AttributeNames.HTTP_METHOD, null)
    0 * _
  ...
  def "test onConnection"() {
    ...
    then:
    if (conn) {
      1 * span.setAttribute(AttributeNames.PEER_HOSTNAME, "test-host")
      1 * span.setAttribute(AttributeNames.PEER_PORT, 555)
      if (ipv4) {
        1 * span.setAttribute(AttributeNames.PEER_HOST_IPV4, "10.0.0.1")
      } else if (ipv4 != null) {
        1 * span.setAttribute(AttributeNames.PEER_HOST_IPV6, "3ffe:1900:4545:3:200:f8ff:fe21:67cf")
      }
    }
    0 * _
